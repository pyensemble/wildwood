...
def logistic_regression(
    ...
    return [fit_time] + datasets.evaluate_classifier(
        clf, dataset.data_test, dataset.target_test, binary=dataset.binary
    )
...
def sklearn_random_forest(
    dataset, n_estimators=100, n_jobs=-1, criterion="gini", random_state=0
):
...
def wildwood(dataset, n_estimators=100, n_jobs=-1, criterion="gini", random_state=0):
    ...
    clf.fit(
        dataset.data_train[:100], dataset.target_train[:100]
...
if all_datasets:
    dataset_names = [
        ...
        "KDDCup",
        "NewsGroups",
    ] + dataset_names  # take the biggest ones first in case there are data loading issues ...
    print(
        "Using ALL datasets for benchmarking including : Higgs, Covtype, KDDCup, NewsGroups"
    )
