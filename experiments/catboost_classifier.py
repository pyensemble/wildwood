...
if (cat_features is not None) and len(cat_features) > 0 and use_cat_features:
...
tic = time()
clf.fit(train_pool, dataset.target_train, verbose=bool(args.verbose), sample_weight=sample_weights, cat_features=cat_features)
toc = time()
