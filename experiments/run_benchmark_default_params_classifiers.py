...
"""
This script produces experiments with defaults hyper-parameters (Table 2 and 4) from the WildWood's paper.
...
def run_default_params_exp(
    ...
    for rep, fit_seed in enumerate(fit_seeds):
        ...
        tic = timer()
        clf.fit(X_train, y_train, **fit_kwargs_generator(clf_name, dataset))
        toc = timer()
        fit_time = toc - tic
