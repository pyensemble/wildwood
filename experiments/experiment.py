...
class Experiment(object):
    ...
    def __init__(self, learning_task='classification', bst_name=None, n_estimators=100, hyperopt_evals=50,
                 compute_counters=True, counters_sort_col=None,
                 output_folder_path='./'):
    ...
    def optimize_params(self, X_train, y_train, X_val, y_val, max_evals=None, verbose=True):
        max_evals = max_evals or self.hyperopt_evals
        self.trials = Trials()
        self.hyperopt_eval_num, self.best_loss = 0, np.inf

        _ = fmin(fn=lambda params: self.run(X_train, y_train, X_val, y_val, params, verbose=verbose),
    ...
    def run(self, X_train, y_train, X_val, y_val, params=None, n_estimators=None, verbose=False, cat_cols=None):
        ...
        dtest = self.convert_to_dataset(X_val.astype(float), y_val, cat_cols)
        _, evals_results = self.fit(params, dtrain, dtest, n_estimators)
...
class LGBExperiment(Experiment):
    ...
    def __init__(self, learning_task, n_estimators=100, max_hyperopt_evals=50,
                 counters_sort_col=None, output_folder_path='./'):
        Experiment.__init__(self, learning_task, 'lgb', n_estimators, max_hyperopt_evals,
                            True, counters_sort_col, output_folder_path)
