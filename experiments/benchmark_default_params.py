...
def set_classifier(clf_name, fit_seed, n_jobs=-1):
    classifier_setting = {
        "RandomForestClassifier10": RandomForestClassifier(
            n_estimators=10, n_jobs=n_jobs, random_state=fit_seed
        ),
        "RandomForestClassifier100": RandomForestClassifier(
        ...
        "WildWood10": ForestClassifier(
            n_estimators=10, n_jobs=n_jobs, random_state=fit_seed
        ),
        "WildWood100": ForestClassifier(
            n_estimators=100, n_jobs=n_jobs, random_state=fit_seed
        ),
    }
...
def clf_name_to_extract_key(clf_name):
    if clf_name == "WildWood10" or clf_name == "WildWood100":
        ...
        clf_name == "RandomForestClassifier10"
        or clf_name == "RandomForestClassifier100"
    ):
...
def run_default_params_exp(
    ...
    for rep, fit_seed in enumerate(fit_seeds):
        ...
        if clf_name_to_extract_key(clf_name) == "WildWood":  # pre-compile wildwood
...
if __name__ == "__main__":
    ...
    results = run_default_params_exp(dataset, clf_name, dataset.task)
